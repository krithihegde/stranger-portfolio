...
const bulbColors = ["blue", "yellow", "red"];

letters.forEach((letter, index) => {
  // Remove any existing bulb (safety)
  const existingBulb = letter.querySelector(".bulb");
  if (existingBulb) existingBulb.remove();

  const bulb = document.createElement("div");
  bulb.classList.add("bulb");

  const colorClass = bulbColors[index % bulbColors.length];
...
soundToggle.addEventListener("click", () => {
  ...
  if (!soundEnabled) {
    soundHum.pause();
    soundHum.currentTime = 0;
  } else {
    soundHum.play().catch(()=>{});
  }
});

// Start hum on first interaction
document.body.addEventListener("click", () => {
  if (soundEnabled && soundHum.paused) {
    soundHum.play().catch(()=>{});
  }
}, { once: true });
...
upsideToggle.addEventListener("click", () => {
  ...
  upsideToggle.textContent = document.body.classList.contains("upside-down")
    ? "Return to the Right Side Up"
    : "Enter the Upside Down";
});

// PROJECT DATA (shortened sample — your full one already exists)
const projects = {
  A: {
  ...
  E: {
    ...
    links: [
    { label: "View events", url: "https://www.canva.com/design/DAGqyyFy-Eo/LNEGyexL-OzSfGzUdc2m8Q/edit?utm_content=DAGqyyFy-Eo&utm_campaign=designshare&utm_medium=link2&utm_source=sharebutton" },
    ]
  },
  F: {
...
letters.forEach(letter => {
  ...
  letter.addEventListener("mouseenter", () => {
    ...
    display.textContent = projects[key]?.title || key;
